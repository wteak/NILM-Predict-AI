...
class TrainingHandler():
    def __init__(self, config):
        ...
        if config['wandb']:
            self.project = 'nilm-seq2point'
            self.wandb = wandb.init(
                project=self.project,
            )
            self.wandb.name = f"{get_str_now()}, batch:{config['batch_size']}, lr:{config['lr']}, seq_len:{config['sequence_length']}"
    ...
    def load_state(self, channel, epoch):
        ...
        if not self.current_state_json.exists():
            _temp_json = {
                ...
                'epoch': 0,
                'channel': channel,
                'train_best_mse_loss' : None,
                'train_best_mae_loss' : None,
                'val_best_mae_loss' : None,
                'train_mse_losses': [],
                'train_mae_losses': [],
                'val_mae_losses': [],
                'last_ckp': None
    ...
    def save_model_ckp(self, model):
        ...
        if not ckp_subpath.exists():
            ckp_subpath.mkdir(exist_ok=True, parents=True)
        torch.save(model.state_dict(), ckp_subpath / f"{channel:02d}-epoch-{epoch:03}.pth")

        ckp_file = str((ckp_subpath / f"{channel:02d}-epoch-{epoch:03}.pth"))
...
def make_training_data_split(config):
    ...
    for f in f_list:
        ...
        if channel != 'ch01':
            ...
            # ch01 분전반 데이터 pair
            parents = f.parent
            main_file_name = f"{user_id}_ch01_{collected_date}.parquet"
            data_dict['main_path'] = Path(parents) / main_file_name
